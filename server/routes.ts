...
const JWT_SECRET =
  process.env.JWT_SECRET || "ilava-jwt-secret-fallback-key-2024";
const SESSION_SECRET =
  process.env.SESSION_SECRET || "ilava-session-secret-fallback-key-2024";
...
export function registerRoutes(app: Express): Server {
      ...
      try {
        ...
        if (!user) {
          ...
          if (!userType) {
            return res
              .status(400)
              .json({ error: "User type is required for new registrations" });
          }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error(
            "Database error during Google auth, using fallback:",
            dbError,
          );
  ...
  app.post("/api/ai-assistant", async (req, res) => {
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a helpful assistant specialized in organic waste marketplace queries.",
            },
      ...
      res.status(500).json({
        error: "Failed to process AI request using Microsoft Phi API.",
      });
    }
  });
